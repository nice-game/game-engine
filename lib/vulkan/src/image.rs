pub use ash::vk::{Format, ImageLayout, ImageSubresourceRange, ImageType, ImageUsageFlags};
...
impl Image {
	pub fn init(
		...
		let buf = Arc::new(Self { device, vk, alloc, width, height, depth, layout: Atomic::new(layout) });
		ImageInit::new(buf)
	}
...
impl ImageInit {
	pub fn copy_from_buffer<T: 'static>(
		self,
		queue: &Arc<Queue>,
