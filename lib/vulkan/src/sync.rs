...
impl Fence {
	pub fn new(mut prev: impl GpuFuture + 'static) -> Self {
		...
		Self { prev: Box::new(prev), vk }
	}

	pub fn wait(&self) {
		unsafe { self.prev.device().vk.wait_for_fences(&[self.vk], false, !0) }.unwrap();
	}
}
impl Drop for Fence {
	fn drop(&mut self) {
		self.wait();
		unsafe { self.prev.device().vk.destroy_fence(self.vk, None) };
	}
}
