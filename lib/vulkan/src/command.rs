...
impl<SEC: Bit> CommandBufferBuilder<SEC> {
	...
	pub fn copy_buffer_to_image<T: 'static>(mut self, src: Arc<Buffer<[T]>>, dst: Arc<Image>) -> Self {
		...
		let image_subresource = vk::ImageSubresourceLayers::builder()
			...
			.layer_count(1)
			.build();
		let image_extent = vk::Extent3D::builder().width(dst.width()).height(dst.height()).depth(dst.depth()).build();
